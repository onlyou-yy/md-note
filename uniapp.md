...
uniapp 除了有web端的一些交互事件之外，还要移动端的事件

+ `@input`：输入事件
+ `@focus`：获取焦点事件
+ `@blur`：输入框失去焦点事件
+ `@confirm`：移动端输入键盘的完成 / 回车键被点击
+ `@tab`：触摸事件。类似于click事件，如果在web端同时定义了点击事件和tab事件，那么tab事件会覆盖click事件，但是在移动端的话click 事件和 tab事件 都会被触发
+ `@longpress`：长按事件，触摸超过350ms事件将会被触发
+ `@longtap`：长触摸事件，如果同时绑定了tap事件，并且触发了longtap 事件，那么tap事件将不会被触发，并且如果同时绑定了 longpress 和 longtap 事件，那个longtap事件将会吧longpress覆盖。（不推荐使用）
+ `@touchstart`：触摸动作开始
+ `@touchend`：触摸动作结束
+ `@touchcancel`：触摸动作取消，当有其他事件打断了触摸事件，如来电提醒，弹窗
+ `@touchmove`：触摸后移动
...
uniapp 已经将常用的 js api和 ui 组件都封装到了框架中了，一般情况下都是可以兼容多平台的，但是有时候自己想要控制在不同的端中使用不同的代码的话，就需要使用条件编译

uniapp 中参考了 c 语言中的 #ifdef、#ifndef 的方式来进行控制编译

**写法**：以 #ifdef 或者 #ifndef 加 %PLATFORM% 开头，以 #endif 结尾。

+ `#ifdef`：if defined 仅在某平台存在
+ `#idndef`：if  not defined 除了某平台均存在
+ `%PLATFORM%`：平台名称
...
<template>
    ...
    <!-- #ifndef MP -->
    <view>不在小程序编译，在H5 / ios / 安卓只编译</view>
    <!-- #endif -->
  </view>
</template>
```
