...
假设设计稿宽度是 750px，通常希望 1rem 对应 100px，这样就可以根据屏幕宽度动态调整 `font-size`：。当屏幕大小发生变化时（移动端禁用缩放后不可能出现这种情况，可以在初始化的时候设置）设置html的font-size。
...
`手机中元素的宽度 = 设计稿元素的宽度 * 手机屏宽 / 设计稿的宽度 `

```js
function px2rem(UIElWidth){
    return UIElWidth * phoneWidth / UIWidth;
}
```
...
[vfit](https://vfit.raychart.cn/)

[数字孪生大屏 可视化适配方案](https://vfit-adapt.raychart.cn/)